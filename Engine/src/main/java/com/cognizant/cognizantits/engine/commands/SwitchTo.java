...
public class SwitchTo extends Command {
     ...
     * **************************************
     */
    @Action(object = ObjectType.BROWSER, desc = "Switch to parent by name: [<Data>]", input = InputType.YES)
    public void switchToParentFrame() {
        try {
            Driver.switchTo().parentFrame();
            Report.updateTestLog(Action, "Webdriver switched to parent frame", Status.DONE);
        } catch (Exception e) {
            Logger.getLogger(this.getClass().getName()).log(Level.OFF, null, e);
            Report.updateTestLog(Action, e.getMessage(), Status.DEBUG);
        }
